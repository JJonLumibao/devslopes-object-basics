// Module 3 Exercise: Looping in Objects
// Start with exercise-3.md if you haven't read it!

const dominos = {
  name: "Domino's",
  cuisines: ["italian", "new york", "pizza"],
  pizzaToppings: {
    pepperoni: 2.2,
    mushrooms: 0.7,
    extraCheese: 1.4,
    onions: 0.5,
    sausage: 2.8,
    extraSauce: 0.8,
  },
  starReviews: {
    Peach: 3,
    Yoshi: 2.1,
    Bowser: 4,
    Mario: 5,
    Luigi: 5,
    Gumba564: 3.4,
    "Donkey Kong": 3.2,
  },
  favorited: true,
  address: "123 Elm Street",
  zipcode: 54321,
  acceptsReservations: true,
};

const printPizzaPlace = (obj) => {
  for(const key in obj) {
    console.log(key, obj[key]);
  }
}
printPizzaPlace(dominos);

// const toppingsPriceRange = (obj) => {
//   const firstKey = Object.keys(obj.pizzaToppings)[0];
//   let highest = obj.pizzaToppings[firstKey];
//   let lowest = obj.pizzaToppings[firstKey];
//   for(const key in obj.pizzaToppings) {
//     const price = obj.pizzaToppings[key];
//     if(price > highest) {
//       highest = price;
//     } else if(price < lowest) {
//       lowest = price;
//     } 
//   }
//   return [lowest, highest];
// }

const toppingsPriceRange = (obj) => {
  const prices = Object.values(obj.pizzaToppings);
  return [Math.min(...prices), Math.max(...prices)];
}

console.log(toppingsPriceRange(dominos));

const calculateAverageRating = (obj) => {
  const reviews = Object.values(obj.starReviews);
  let total = 0;
  for(const value of reviews) {
    total += value;
  }
  return (total / reviews.length).toFixed(2);
}

console.log(calculateAverageRating(dominos));
